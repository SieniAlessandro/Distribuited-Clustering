...
def removeOldFiles():
    ...
    if os.path.exists(DATA_SINK_FOLDER):
        ...
        dirnames = [f for f in listdir(DATA_SINK_FOLDER) if isdir(join(DATA_SINK_FOLDER, f))]
        #Deleting the files
        for filename in filenames:
            os.remove(DATA_SINK_FOLDER+"/"+filename)
        #Deleting the directory
        for dirname in dirnames:
            shutil.rmtree(DATA_SINK_FOLDER+"/"+dirname)
...
def save(type,id,fileName,jsonToSave):
    ...
    if type == 0:
        ...
        with open(LOG_DIR_PATH+"/"+fileName+".json","w") as fileToSave:
            json.dump(jsonToSave,fileToSave)
    else:
        LOG_DIR_PATH =  SINK_LOG_BASE_FOLDER
        if not os.path.exists(LOG_DIR_PATH):
            os.makedirs(LOG_DIR_PATH)
        with open(LOG_DIR_PATH+"/"+fileName+".json","w") as fileToSave:
            json.dump(jsonToSave,fileToSave)
